...
public interface SkupinaRepository extends CrudRepository<Skupina, Long> {
    ...
    @Query("SELECT sp.id, sp.telo FROM skupina sk INNER JOIN sporocilo sp ON sk.id = sp.skupina_id WHERE skupina_id =2 GROUP BY sp.id;")
    ...
    @Query(value = "SELECT sp FROM sporocilo sp WHERE sp.skupina_id =  ", nativeQuery = true)
    List<Sporocilo> vrnem(int st_sporocil);
